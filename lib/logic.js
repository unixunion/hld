 ...
 * @param {org.kindred.hldemo.DebitTransaction} debitTransaction
 * @transaction
 */
async function debitTransaction(tx) {
    ...
    if (newValue<0) {
        ...
        const assetRegistry = await getAssetRegistry('org.kindred.hldemo.AccountBalanceAsset');
        // Update the asset in the asset registry.
        await assetRegistry.update(tx.asset);

        // Emit an event for the modified asset.
        let event = getFactory().newEvent('org.kindred.hldemo', 'BalanceChangedEvent');
...
/**
 * credit transaction
 * @param {org.kindred.hldemo.CreditTransaction} creditTransaction
 * @transaction
 */
 ...
 * @param {org.kindred.hldemo.TransferTransaction} transferTransaction
 * @transaction
 */
async function transferTransaction(tx) {
    ...
    if ( tx.amount>0 && tx.amount <= fromBalance ) {
        ...
        let event1 = getFactory().newEvent('org.kindred.hldemo', 'BalanceChangedEvent');
        ...
        let event2 = getFactory().newEvent('org.kindred.hldemo', 'BalanceChangedEvent');
